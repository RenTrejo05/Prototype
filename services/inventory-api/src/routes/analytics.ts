...
export async function getAnalytics(_req: Request, res: Response): Promise<void> {
  try {
    ...
    const totalStockUnits = products.reduce((s: number, p: Product) => s + p.stock, 0);
    const totalStockValue = products.reduce((s: number, p: Product) => s + p.stock * p.cost, 0);
    ...
    const suggestedRestocks: { product: Product; suggestedQty: number }[] = lowStockProducts.map(
      (p: Product) => ({ product: p, suggestedQty: Math.max(p.minStock * 2 - p.stock, 1) }),
    );
